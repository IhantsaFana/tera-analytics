...
def graphique_dashboard(df, stats, moyenne_journaliere, mode="show"):
    """Construit un dashboard riche ou exporte plusieurs graphiques séparés."""
    style()
    ...
    # If exporting, create and save individual figures instead of one assembled dashboard
    if mode == "export":
        chemins = []
        ...
        ax.annotate(f"Pic à {peak_hour}h", xy=(peak_hour, peak_hour_value), xytext=(peak_hour + 1.5, peak_hour_value + 2), arrowprops=dict(arrowstyle="->", color="black"), fontsize=8)
        ax.set(xlabel="Heure", ylabel="PM2.5 (µg/m³)", title="Cycle diurne")
        ax.legend(loc="best")
        fig.tight_layout()
        chemins.append(finaliser(fig, "profil_horaire", mode))
        ...
        ax.annotate(f"Max : {mois[peak_month - 1]}", xy=(peak_month, peak_month_value), xytext=(peak_month + 0.2, peak_month_value + 1.5), arrowprops=dict(arrowstyle="->", color="black"), fontsize=8)
        ax.set(xlabel="Mois", ylabel="PM2.5 (µg/m³)", title="Variation saisonnière")
        ax.legend(loc="best")
        fig.tight_layout()
        chemins.append(finaliser(fig, "profil_mensuel", mode))
        ...
        ax.legend(loc="best")
        fig.tight_layout()
        chemins.append(finaliser(fig, "serie_temporelle", mode))
        ...
        ax.legend(loc="best")
        fig.tight_layout()
        chemins.append(finaliser(fig, "capteurs", mode))
        ...
        ax.legend(loc="best")
        fig.tight_layout()
        chemins.append(finaliser(fig, "distribution", mode))
        ...
        fig.tight_layout()
        chemins.append(finaliser(fig, "boxplot_mensuel", mode))
        ...
        fig.tight_layout()
        chemins.append(finaliser(fig, "boxplot_horaire", mode))

        # Relation PM1 vs PM2.5
        fig, ax = plt.subplots()
        ...
        fig.tight_layout()
        chemins.append(finaliser(fig, "pm1_vs_pm25", mode))

        # Taux de dépassement par mois
        fig, ax = plt.subplots()
        ...
        ax.legend(loc="best")
        fig.tight_layout()
        chemins.append(finaliser(fig, "taux_depassement_mensuel", mode))

        return chemins

    # Default: return a single assembled dashboard figure
